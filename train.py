...
class Train:
    def __init__(self,root): 
        ...
        img_top = img_top.resize((1280,260),Image.ANTIALIAS)
        self.photoimg_top = ImageTk.PhotoImage(img_top)
        lb_top = Label(self.root,image=self.photoimg_top)
        lb_top.place(x=0, y=45, width=1280, height=260)
        
        #button
        b1_1=Button(self.root,text="TRAIN DATA",command=self.train_classifier,cursor="hand2",font=("times new roman",30,"bold"),bg="red", fg="white")
        b1_1.place(x=0,y=300,width=1280,height=60)

        img_bot = Image.open(r"Photos\faces3.jpg")
        img_bot = img_bot.resize((1280,400),Image.ANTIALIAS)
        self.photoimg_bot = ImageTk.PhotoImage(img_bot)
        lb_bot = Label(self.root,image=self.photoimg_bot)
        lb_bot.place(x=0, y=360, width=1280, height=400)
